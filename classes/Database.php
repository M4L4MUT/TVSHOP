...
class Database {
   ...
   public function __construct($host, $username, $pass, $db) {
    ...
    public function megrendeles($tv_modell, $marka, $gyartasd, $kijelzom, $allapot, $ar) {
        $stmt = $this->db->prepare("DELETE FROM tv WHERE `tv`.`tvid` = $tvid");
        $stmt->bind_param('ssssss', $tvid, $tv_modell, $marka, $gyartasd, $kijelzom, $allapot, $ar);
        return $stmt->execute();
    }
    public function feltoltesTV($tv_modell, $marka, $gyartasd, $kijelzom, $allapot, $ar) {
        $stmt = $this->db->prepare("INSERT INTO `tv`(`tv_modell`, `marka`, `gyartasd`, `kijelzom`, `allapot`, `ar`) VALUES (?,?,?,?,?,?");
        $stmt->bind_param("ssssss", $tv_modell, $marka, $gyartasd, $kijelzom, $allapot, $ar);
        $stmt->execute();
    }
